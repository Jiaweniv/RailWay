...
(function() {
	...
	var setting = {
		...
		callback: {
			onClick: function(event, treeId, treeNode) {
				...
				$("#tiezongjiandujigou input[name='search_name']").val("");
				var orgid_jdzxs = [];
				...
				$('#tiezongjiandujigou_table').bootstrapTable("refreshOptions", {
					pageNumber: 1
	...
	Mydao.ajax({
		...
		if(result.code == 200) {
			...
			var treeObj = $.fn.zTree.init($("#tiezongjiandujigou #tiezongjiandujigou_tree"), setting, zNodes);
	 ...
	 * 添加
	 */
	$('#tiezongjiandujigou_lzh').find('#tiezongjiandujigou_menu_add').on('click', function() {
		$('#bumennameOut').show();
		$('#bumenjcOut').hide();
		var treeObj = $.fn.zTree.getZTreeObj("tiezongjiandujigou_tree");
					...
					if(result.code == 200) {
						...
						Mydao.ajax({
							...
							if(result.code == 200) {
								...
								$.fn.zTree.destroy("tiezongjiandujigou_tree");
								$.fn.zTree.init($("#tiezongjiandujigou #tiezongjiandujigou_tree"), setting, zNodes);
							} else {
								layer.alert(result.msg);
							}
	...
	//查询
	$('#tiezongjiandujigou_lzh').find("#tiezongjiandujigou button[name='search']").on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("tiezongjiandujigou_tree");
		var _selectNodes = treeObj.getSelectedNodes(),
			treeNode = {};
		...
		Mydao.currentPage.params.name = $("#tiezongjiandujigou input[name='search_name']").val();
		$('#tiezongjiandujigou_table').bootstrapTable("refreshOptions", {
			pageNumber: 1
		}).bootstrapTable("refresh");
	});
	 ...
	 * 编辑
	 */
	$('#tiezongjiandujigou_lzh').find('#tiezongjiandujigou_menu_edit').on('click', function() {
		//				if()
		$('#bumennameOut').show();
		$('#bumenjcOut').hide();
		var treeObj = getSelectedTreeNode("tiezongjiandujigou_tree");
		if(!treeObj) {
			layer.msg('请选择修改项');
			return false;
		}
			...
			yes: function(index, layero) {
				...
				Mydao.ajax({
					...
					if(data.code == 200) {
						...
						$.fn.zTree.getZTreeObj('tiezongjiandujigou_tree').updateNode(treeObj);
					} else {
						layer.msg(data.msg);
					}
				});
	 ...
	 * 删除
	 */
	$('#tiezongjiandujigou_lzh').find('#tiezongjiandujigou_menu_del').on('click', function() {
		var treeObj = getSelectedTreeNode('tiezongjiandujigou_tree');
		...
		layer.confirm('确定删除？', function(index) {
			$(this).doajax({
				...
				$.fn.zTree.getZTreeObj('tiezongjiandujigou_tree').removeNode(treeObj);
			});
			layer.close(index);
		});
	});
	...
	///////////////////////////人员信息////////////////////////
	//		新建人员
	$('#tiezongjiandujigou_lzh').find('#personnel_btn_tzjdjg').on('click', function() {
		layer.open({
			type: 1,
			title: '新建人员',
			content: '',
						...
						layui.use(['laytpl'], function() {
							...
							layero.find('.layui-layer-content').load("view/InstitutionalPersonnel/PersonneChilds/PersonSee.html", function() {
								...
								if(identityback) {
									...
									for(var f = 0; f < identityback_array.length; f++) {
										$('#idcardfile').append("<img src='" + MydaoFileDownPath + "?fileId=" + identityback_array[f] + "' width='100px'/>");
									}
								}

								if(jobentityback) {
									var jobentityback_array = jobentityback.split(",");
									for(var q = 0; q < jobentityback_array.length; q++) {
										$('#jobtitlefile').append("<img src='" + MydaoFileDownPath + "?fileId=" + jobentityback_array[q] + "' width='100px'/>");
									}
								}
								if(offientityback) {
									var offientityback_array = offientityback.split(",");
									for(var w = 0; w < offientityback_array.length; w++) {
										$('#officefile').append("<img src='" + MydaoFileDownPath + "?fileId=" + offientityback_array[w] + "' width='100px'/>");
									}
								}
	...
	function personList() {
		$('#tiezongjiandujigou_table').bootstrapTable({
			pagination: true,
