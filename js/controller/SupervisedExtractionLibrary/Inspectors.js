				...
				yes: function(index, layero) { //回调
					Mydao.ajax({
						...
						if(result.code == 200) {
							layer.alert("操作成功！");
							//刷新页面
							$("#inspectors #inspectors_table").bootstrapTable("refreshOptions", {
								pageNumber: 1
...
//				},
		...
		Mydao.ajax({
			...
			if(result.code == 200) {
				//刷新页面
				$("#inspectors #inspectors_profession").append('<option value="">--所有专业--</option>');
				$.each(result.result, function(index, val) {
					$("#inspectors #inspectors_profession").append('<option value="' + val.id + '">' + val.name + '</option>');
				});
			} else {
		...
		//表格
		var url = "person/findCheckUser";
		$('#inspectors #inspectors_table').bootstrapTable({
			pagination: true,
			...
			queryParams: function(p) {
				...
				Mydao.config.ajaxParams.params.profession = $('#inspectors [name="profession"]').val();
				Mydao.config.ajaxParams.params.name = $('#inspectors [name="name"]').val();
				return Mydao.config.ajaxParams;
			},
			columns: [{
				title: '序号',
	...
	//查询
	$('#inspectors #inspectors_search').on('click', function(event) {
		$("#inspectors #inspectors_table").bootstrapTable("refreshOptions", {
			pageNumber: 1
		}).bootstrapTable("refresh");
	});
