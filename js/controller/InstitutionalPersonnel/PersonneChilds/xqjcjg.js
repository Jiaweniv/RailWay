...
(function() {
	...
	var setting = {
		...
		callback: {
			onClick: function(event, treeId, treeNode) {
				...
				$("#xiaqujiancejigou input[name='search_name']").val("");
				var orgid_jdzxs = [];
				orgid_jdzxs.push(-1);
				...
				$('#xiaqujiancejigou_table').bootstrapTable("refreshOptions", {
					pageNumber: 1
	...
	var zNodes = [];
	Mydao.ajax({}, 'organization/findXQJCJG', function(result) {
		if(result.code == 200) {
			...
			var treeObj = $.fn.zTree.init($("#xiaqujiancejigou #xiaqujiancejigou_tree"), setting, zNodes);
	 ...
	 * 添加
	 */
	$('#xiaqujiancejigou_lzh').find('#xiaqujiancejigou_menu_add').on('click', function() {
		$('#bumennameOut').hide();
		$('#bumenjcOut').show();
		var treeObj = $.fn.zTree.getZTreeObj("xiaqujiancejigou_tree");
		var _selectNodes = treeObj.getSelectedNodes();
		var _selectNodesID;
		if(_selectNodes.length == 1) {
			if(_selectNodes[0].cando == 0) {
				layer.msg("该节点不能添加部门");
				return false;
			}
			_selectNodesID = _selectNodes[0].id;
		} else {
			layer.msg("请选择要添加部门的检测单位或部门");
			return false;
		}
		var dialog = layer.open({
			...
			yes: function(index, layero) {
				...
				Mydao.ajax({
					...
					"abbreviation": layero.find('form [name="abbreviation"]').val(),
					...
					if(result.code == 200) {
						...
						Mydao.ajax({}, 'organization/findXQJCJG', function(result) {
							if(result.code == 200) {
								...
								$.fn.zTree.destroy("xiaqujiancejigou_tree");
								$.fn.zTree.init($("#xiaqujiancejigou #xiaqujiancejigou_tree"), setting, zNodes);
							} else {
								layer.alert(result.msg);
							}
	...
	//查询
	$('#xiaqujiancejigou_lzh').find("#xiaqujiancejigou button[name='search']").on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("xiaqujiancejigou_tree");
		var _selectNodes = treeObj.getSelectedNodes(),
			treeNode = {};
		...
		Mydao.currentPage.params.name = $("#xiaqujiancejigou input[name='search_name']").val();
		$('#xiaqujiancejigou_table').bootstrapTable("refreshOptions", {
			pageNumber: 1
		}).bootstrapTable("refresh");
	});
	 ...
	 * 编辑
	 */
	$('#xiaqujiancejigou_lzh').find('#xiaqujiancejigou_menu_edit').on('click', function() {
		$('#bumennameOut').hide();
		$('#bumenjcOut').show();
		var treeObj = getSelectedTreeNode("xiaqujiancejigou_tree");
		if(!treeObj) {
			layer.msg('请选择修改项');
			return false;
		}
		if(treeObj.level == 0 || treeObj.level == 1) {
			layer.msg('不能编辑该机构');
			return false;
		}
		var dialog = layer.open({
			...
			success: function(layero, index) {
				Mydao.ajax({
					...
					if(data.code == 200) {
						...
						layero.find('#abbreviation').val(result.abbreviation);
					}
				});
			...
			yes: function(index, layero) {
				...
				Mydao.ajax({
					...
					if(data.code == 200) {
						...
						treeObj.abbreviation = layero.find('form [name="abbreviation"]').val();
						$.fn.zTree.getZTreeObj('xiaqujiancejigou_tree').updateNode(treeObj);
					} else {
						layer.msg(data.msg);
					}
				});
	 ...
	 * 删除
	 */
	$('#xiaqujiancejigou_lzh').find('#xiaqujiancejigou_menu_del').on('click', function() {
		var treeObj = getSelectedTreeNode('xiaqujiancejigou_tree');
		if(!treeObj) {
			layer.msg('请选择删除项');
			return false;
		...
		layer.confirm('确定删除？', function(index) {
			$(this).doajax({
				...
				$.fn.zTree.getZTreeObj('xiaqujiancejigou_tree').removeNode(treeObj);
			});
			layer.close(index);
		});
	});
	...
	///////////////////////////人员信息////////////////////////
	//		新建人员
	$('#xiaqujiancejigou_lzh').find('#personnel_btn_xqjcjg').on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("xiaqujiancejigou_tree");
		var _selectNodes = treeObj.getSelectedNodes();
		if(_selectNodes.length == 1) {
			if(_selectNodes[0].cando == 0) {
		...
		layer.open({
			...
			success: function(layero, index) {
				layero.find('.layui-layer-content').load("view/InstitutionalPersonnel/PersonneChilds/departmentlayer.html", function() {
					$("#Personnel_classification").attr("data-params", "{'type':'PTYPEF'}");
						...
						layui.use(['laytpl'], function() {
							...
							layero.find('.layui-layer-content').load("view/InstitutionalPersonnel/PersonneChilds/PersonSee.html", function() {
								...
								if(identityback) {
									...
									for(var h = 0; h < identityback_array.length; h++) {
										$('#idcardfile').append("<img src='" + MydaoFileDownPath + "?fileId=" + identityback_array[h] + "' width='100px'/>");
									}
								}

								if(jobentityback) {
									var jobentityback_array = jobentityback.split(",");
									for(var j = 0; j < jobentityback_array.length; j++) {
										$('#jobtitlefile').append("<img src='" + MydaoFileDownPath + "?fileId=" + jobentityback_array[j] + "' width='100px'/>");
									}
								}
								if(offientityback) {
									var offientityback_array = offientityback.split(",");
									for(var k = 0; k < offientityback_array.length; k++) {
										$('#officefile').append("<img src='" + MydaoFileDownPath + "?fileId=" + offientityback_array[k] + "' width='100px'/>");
									}
								}
								...
								//																$('#DepartmentLayer #Qualificationtable').bootstrapTable('hideColumn', "flag");
							});
						});
	...
	function personList() {
		$('#xiaqujiancejigou_table').bootstrapTable({
			pagination: true,
