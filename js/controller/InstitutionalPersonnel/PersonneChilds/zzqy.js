...
(function() {
	...
	var setting = {
		...
		callback: {
			onClick: function(event, treeId, treeNode) {
				...
				$("#zhuzaiqiye input[name='search_name']").val("");
				var orgid_jdzxs = [];
				orgid_jdzxs.push(-1);
				...
				$('#zhuzaiqiye #zhuzaiqiye_table').bootstrapTable("refreshOptions", {
					pageNumber: 1
	...
	Mydao.ajax({
		"type": 6
	}, 'organization/finddepzzqy', function(result) {
		if(result.code == 200) {
			...
			var treeObj = $.fn.zTree.init($("#zhuzaiqiye #zhuzaiqiye_tree"), setting, zNodes);
	 ...
	 * 添加
	 */
	$('#zhuzaiqiye_lzh').find('#zhuzaiqiye_menu_add').on('click', function() {
		$('#bumennameOut').hide();
		$('#bumenjcOut').show();
		var treeObj = $.fn.zTree.getZTreeObj("zhuzaiqiye_tree");
					...
					if(result.code == 200) {
						...
						Mydao.ajax({
							...
							if(result.code == 200) {
								...
								$.fn.zTree.destroy("zhuzaiqiye_tree");
								$.fn.zTree.init($("#zhuzaiqiye #zhuzaiqiye_tree"), setting, zNodes);
							} else {
								layer.alert(result.msg);
							}
	...
	//查询
	$('#zhuzaiqiye_lzh').find("#zhuzaiqiye button[name='search']").on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("zhuzaiqiye_tree");
		var _selectNodes = treeObj.getSelectedNodes(),
			treeNode = {};
		...
		Mydao.currentPage.params.name = $("#zhuzaiqiye input[name='search_name']").val();
		$('#zhuzaiqiye_table').bootstrapTable("refreshOptions", {
			pageNumber: 1
		}).bootstrapTable("refresh");
	});
	 ...
	 * 编辑
	 */
	$('#zhuzaiqiye_lzh').find('#zhuzaiqiye_menu_edit').on('click', function() {
		$('#bumennameOut').hide();
		$('#bumenjcOut').show();
		var treeObj = getSelectedTreeNode("zhuzaiqiye_tree");
		if(!treeObj) {
			layer.msg('请选择修改项');
			return false;
		}
			...
			yes: function(index, layero) {
				...
				Mydao.ajax({
					...
					if(data.code == 200) {
						...
						$.fn.zTree.getZTreeObj('zhuzaiqiye_tree').updateNode(treeObj);
					} else {
						layer.msg(data.msg);
					}
				});
	 ...
	 * 删除
	 */
	$('#zhuzaiqiye_lzh').find('#zhuzaiqiye_menu_del').on('click', function() {
		var treeObj = getSelectedTreeNode('zhuzaiqiye_tree');
		if(!treeObj) {
			layer.msg('请选择删除项');
			return false;
		...
		layer.confirm('确定删除？', function(index) {
			$(this).doajax({
				...
				$.fn.zTree.getZTreeObj('zhuzaiqiye_tree').removeNode(treeObj);
			});
			layer.close(index);
		});
	});
	...
	///////////////////////////人员信息////////////////////////
	//		新建人员
	$('#zhuzaiqiye_lzh').find('#personnel_btn_zzqy').on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("zhuzaiqiye_tree");
		var _selectNodes = treeObj.getSelectedNodes();
		var _selectNodesID;
		if(_selectNodes.length == 1) {
		...
		$("#example-embed").steps({
			...
			onFinished: function(event, currentIndex) {
				...
				if(!paramsPerson.id) {
					...
					var _url = 'personAptitude/edit';
					params.personid = $("input[name='id']").val();
					params.paramsAptitude = paramsAptitude;
					Mydao.ajax(params, _url, function(result) {
						if(result.code == 200) {
							layer.alert("操作成功！");
							layer.close(Mydao.dialogid_jdzx);
							$('#zhuzaiqiye_table').bootstrapTable("refreshOptions", {
						...
						layui.use(['laytpl'], function() {
							...
							layero.find('.layui-layer-content').load("view/InstitutionalPersonnel/PersonneChilds/PersonSee.html", function() {
								...
								if(identityback) {
									...
									for(var o = 0; o < identityback_array.length; o++) {
										$('#idcardfile').append("<img src='" + MydaoFileDownPath + "?fileId=" + identityback_array[o] + "' width='100px'/>");
									}
								}

								if(jobentityback) {
									var jobentityback_array = jobentityback.split(",");
									for(var u = 0; u < jobentityback_array.length; u++) {
										$('#jobtitlefile').append("<img src='" + MydaoFileDownPath + "?fileId=" + jobentityback_array[u] + "' width='100px'/>");
									}
								}
								if(offientityback) {
	...
	function personList() {
		$('#zhuzaiqiye_table').bootstrapTable({
			pagination: true,
	...
	var checkPerson = function() {
		$("select[name='checkflag']").change(function() {
			...
			if(val == 1) {
				$("#law").show();
				$("#law").find("input").css("width", "220px");
				$("#law").find("input").removeAttr("disabled");
			} else {
				$("#law").hide();
				$("#law").find("input").attr("disabled", "disabled");
			}
		});
	};
