...
layui.define('layer', function(exports){
  ...
  Upload.prototype.init = function(){
    ...
    return elem.each(function(index, item){
      ...
      form = $(form);
      form.validator&&form.validator( "destroy" );
      //拖拽支持
      if(!options.unwrap){
  ...
  Upload.prototype.action = function(input, type){
    ...
    if((options.before && options.before(input))===false){
    	return false;
    }
    item.parent().parent().children().append($('<input name="userid" value="'+Mydao.config.ajaxParams.base.userid+'" hidden="hidden"/>')).append($('<input name="clientid" value="'+Mydao.config.ajaxParams.base.clientid+'" hidden="hidden"/>'));
    item.parent().submit();
	$("span[class='layui-upload-icon'] input").remove();
	item.parent().parent().children().find("input[name='userid'],input[name='clientid']").remove();
