	...
	(function() {
					...
					if(result.code == 200) {
						...
						layer.open({
							...
							yes: function(index, layero) { //回调
								...
								}, 'supervisionReport/edit', function(result) {
									layer.close(index); //如果设定了yes回调，需进行手工关闭
									var currentTime = result.serverTime;
									if(result.code == 200) {
										layer.alert("操作成功！");
										//刷新页面
										$("#SupervisoryWorkReport #xmztbg_table4").bootstrapTable("refreshOptions", {
											pageNumber: 1
		...
		$('#SupervisoryWorkReport #xmztbg_table4').bootstrapTable({
			pagination: true,
			...
			queryParams: function(p) {
				...
				Mydao.config.ajaxParams.params.type = 4;
