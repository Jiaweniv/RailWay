(function() {
					...
					success: function(layero, index) {
						layero.find('.layui-layer-content').load('view/LawOperations/ComplaintChildr.html', function() {
							...
							Mydao.initselect(layero, null, function() {
								Mydao.setform(layero, resultForm); //填充表单的值
								editer.ready(function() {
									editer.execCommand('inserthtml', resultForm.content, true);
									editer.setDisabled();
								});
							}); //加载select
	...
	$('#ComplaintReport_wtfk #wtfk_table').bootstrapTable({
		pagination: true,
		...
		queryParams: function(p) {
			...
			Mydao.config.ajaxParams.params.reporttype = "1";
		...
		columns: [{
			...
			formatter: function(value, row, index) {
				var ctrls = [];
				if(Mydao.permissions['penalties_edit']) {
					ctrls.push('edit');
				}
				//删除
				if(Mydao.permissions['penalties_del']) {
					ctrls.push('del');
				}
				if(Mydao.permissions['penalties_look']) {
					ctrls.push('view');
				}
