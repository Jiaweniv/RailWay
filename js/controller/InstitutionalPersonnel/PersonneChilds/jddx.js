...
(function() {
	...
	var setting = {
		...
		callback: {
			onClick: function(event, treeId, treeNode) {
				...
				$("#jianduduixiang input[name='search_name']").val("");
				var orgid_jdzxs = [];
				orgid_jdzxs.push(-1);
				...
				$('#jianduduixiang_table').bootstrapTable("refreshOptions", {
					pageNumber: 1
	...
	var zNodes = [];
	Mydao.ajax({}, 'organization/findJDDX', function(result) {
		if(result.code == 200) {
			...
			var treeObj = $.fn.zTree.init($("#jianduduixiang #jianduduixiang_tree"), setting, zNodes);
	 ...
	 * 添加
	 */
	$('#jianduduixiang_lzh').find('#jianduduixiang_menu_add').on('click', function() {
		$('#bumennameOut').hide();
		$('#bumenjcOut').show();
		var treeObj = $.fn.zTree.getZTreeObj("jianduduixiang_tree");
					...
					if(result.code == 200) {
						...
						Mydao.ajax({}, 'organization/findJDDX', function(result) {
							if(result.code == 200) {
								...
								$.fn.zTree.destroy("jianduduixiang_tree");
								$.fn.zTree.init($("#jianduduixiang #jianduduixiang_tree"), setting, zNodes);
							} else {
								layer.alert(result.msg);
							}
	...
	//查询
	$('#jianduduixiang_lzh').find("#jianduduixiang button[name='search']").on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("jianduduixiang_tree");
		var _selectNodes = treeObj.getSelectedNodes(),
			treeNode = {};
		...
		Mydao.currentPage.params.name = $("#jianduduixiang input[name='search_name']").val();
		$('#jianduduixiang_table').bootstrapTable("refreshOptions", {
			pageNumber: 1
		}).bootstrapTable("refresh");
	});
	 ...
	 * 编辑
	 */
	$('#jianduduixiang_lzh').find('#jianduduixiang_menu_edit').on('click', function() {
		$('#bumennameOut').hide();
		$('#bumenjcOut').show();
		var treeObj = getSelectedTreeNode("jianduduixiang_tree");
		if(!treeObj) {
			layer.msg('请选择修改项');
			return false;
		}
			...
			yes: function(index, layero) {
				...
				Mydao.ajax({
					...
					if(data.code == 200) {
						...
						$.fn.zTree.getZTreeObj('jianduduixiang_tree').updateNode(treeObj);
					} else {
						layer.msg(data.msg);
					}
				});
	 ...
	 * 删除
	 */
	$('#jianduduixiang_lzh').find('#jianduduixiang_menu_del').on('click', function() {
		var treeObj = getSelectedTreeNode('jianduduixiang_tree');
		if(!treeObj) {
			layer.msg('请选择删除项');
			return false;
		...
		layer.confirm('确定删除？', function(index) {
			$(this).doajax({
				...
				$.fn.zTree.getZTreeObj('jianduduixiang_tree').removeNode(treeObj);
			});
			layer.close(index);
		});
	});
	...
	///////////////////////////人员信息////////////////////////
	//		新建人员
	$('#jianduduixiang_lzh').find('#personnel_btn_jddx').on('click', function() {
		var treeObj = $.fn.zTree.getZTreeObj("jianduduixiang_tree");
		var _selectNodes = treeObj.getSelectedNodes();
		if(_selectNodes.length == 1) {
			if(_selectNodes[0].cando == 0) {
	...
	function personList() {
		$('#jianduduixiang_table').bootstrapTable({
			pagination: true,
